...
## Блок “Запись / календарь” (заявка на приём)

Этот блок показывает пользователю доступные слоты по расписанию и отправляет заявку администратору.

### Важное про приватность

Контакты клиентов (телефон/email/Telegram) **нельзя хранить в публичном GitHub репозитории**. Поэтому заявка отправляется через Vercel serverless endpoint `POST /api/booking-request` (без публикации в репо).

### Настройка Telegram (чтобы заявки приходили админу)

1) Создай бота через @BotFather, получи токен.
2) Узнай `chat_id` (например, через бота @userinfobot или временно отправив сообщение и посмотрев updates).
3) На Vercel → Project → Settings → Environment Variables добавь:
